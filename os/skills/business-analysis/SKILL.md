...
1. PANORAMICA
   - Cos'è [topic] nel contesto dell'azienda?
...
- Ci sono limiti? (es. "max 100 PMI per venditore")
- Ci sono eccezioni alle regole? (es. "tranne per i partner Tier 1")
...
**Output finale**: `company/prodotto/analysis/analysis-{slug}.md`
Se approvato — genera PRD con `/product write-spec`
...
| partner_id | FK → Partner | Sì | | Relazione | |
...
|-----------|------|-----|------|
| Appartiene a | N:1 | Partner | Ogni PMI ha un solo partner |
| Ha molti | 1:N | Assessment | Storico degli assessment |

## Diagramma

```mermaid
erDiagram
    PARTNER ||--o{ PMI : "gestisce"
    PMI ||--o{ ASSESSMENT : "ha"
    PMI {
        uuid id PK
        string name
...
- Una PMI non può essere eliminata se ha assessment attivi
```
...
- "E se ci sono 1000 PMI invece di 10?"
- "E se il partner vuole personalizzare X?"
...
**Processo**:
1. Leggi l'analisi fatta (`company/prodotto/analysis/analysis-{slug}.md`)
2. Genera specifica funzionale:

```markdown
# Functional Specification: {Feature}

**Analisi di riferimento**: company/prodotto/analysis/analysis-{slug}.md
...
"Voglio gestire [X]"
       │
       ▼
/product analyze [X]          → Domande interattive, capisco il dominio
       │
       ▼
/product map-process          → Mappo come funziona oggi (opzionale)
/product data-model           → Mappo i dati coinvolti (opzionale)
       │
       ▼
/product requirements-elicitation  → Elicito requisiti con MoSCoW
       │
       ▼
/product gap-analysis         → Confronto AS-IS vs TO-BE
       │
       ▼
/product functional-spec      → Specifica funzionale dettagliata
       │
       ▼
/product write-spec           → PRD per lo sviluppo (già esistente)
       │
       ▼
