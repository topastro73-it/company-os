...
**Obiettivo**: Piattaforma configurata, team partner con accesso, prime PMI caricate.

| # | Task | Owner | Success Criteria | Deadline |
|---|------|-------|-----------------|----------|
| 1.1 | Kickoff call con partner (allineamento aspettative, timeline, KPI) — usare `90-Condivisi/template-deliverable/company-poc-kickoff.pptx` come asset di riferimento se il partner ha richiesto un PoC guidato pre-firma | Sales | Meeting avvenuto, note in CRM | Giorno 1 |
...
| 1.4 | Import prime 5-10 PMI pilota | Partner + PM | PMI caricate, primo scan completato | Giorno 7 |
| 1.5 | Configurazione catalogo servizi partner | PM | Catalogo mappato su remediation | Giorno 10 |
| 1.6 | Test end-to-end: scan → report → proposta | PM + Partner | Flusso completo funzionante | Giorno 14 |

### Deliverable fase
- Tenant white-label attivo
- Almeno 5 PMI pilota con scan completato
...
| 2.2 | Training tecnico: interpretare scan e report | PM/Pre-sales | Team tecnico autonomo | Sett 3 |
...
| 2.6 | Definizione target: lista 20-50 PMI da contattare | Partner + Sales | Lista validata e prioritizzata | Sett 4 |

### Deliverable fase
- Team venditori formato e certificato
- Materiale co-branded pronto
- Pipeline iniziale di 20-50 PMI target
...
| # | Task | Owner | Success Criteria | Deadline |
|---|------|-------|-----------------|----------|
| 3.1 | Lancio campagna outbound su lista PMI | Partner + Sales | Campagna attiva, prime risposte | Sett 5 |
| 3.2 | Primi 10+ assessment gratuiti inviati | Partner | 10 PMI con report ricevuto | Sett 6 |
...
| 4.3 | Setup monitoraggio continuo per PMI attive | PM/Engineering | Dashboard partner attiva | Sett 10 |
...
### Processo
1. Crea file `20-Clienti/{slug}/overview.md` dal template
...
### Output
- File partner creato in `20-Clienti/{slug}/overview.md`
...
| Partner B | SETUP | 2/12 | 40% | Primo scan (giorno 7) | Task 1.4 in ritardo |
```
...
- [ ] 1.4 Import PMI pilota — non iniziato (deadline: 2026-03-07)
...
| **PoC kickoff deck** (asset pre-firma — standard per attivare il PoC guidato di un prospect) | `90-Condivisi/template-deliverable/company-poc-kickoff.pptx` (sorgente: `gen_poc_kickoff_deck.py`) |
| **PoC restitution deck** (esempio: {cliente}, riferimento per restituzione risultati post-PoC) | `20-Clienti/acme/report/` (deck restituzione PoC {cliente} — asset storico da ripubblicare) |
