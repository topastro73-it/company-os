...
Configurazione dichiarata in `config/integrations.yaml` (sezione `erp`, solo nomi di variabili — mai valori):
...
Il database viene caricato dal Google Drive file linkato in `DB_FILE_PATH`.
...
| `sync-invoices` | Sincronizza fatture dall'ERP | `vault/finance/fatturazione-erp.md` |
| `sync-cashflow` | Sincronizza saldi cassa dall'ERP | `vault/finance/cashflow-erp.md` |
| `sync-kpis` | Calcola KPI dai financials ERP | `company/direzione/metrics/kpis-erp.md` |
| `pull` | Scarica entità come JSON | `vault/finance/erp-data/*.json` |
| `snapshot` | Backup completo di tutte le entità | `vault/finance/erp-data/snapshot-YYYY-MM-DD.json` |
...
3. Salva come JSON in `vault/finance/erp-data/`
