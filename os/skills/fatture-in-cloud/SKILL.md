...
**Setup** (nomi variabili dichiarati in `config/integrations.yaml`, sezione `fatture_in_cloud`; valori solo in env locale):
```bash
# ~/.zshrc
...
Configurarle nell'ambiente locale (mai committare i valori; i nomi sono censiti in `config/integrations.yaml`) oppure in shell:
```bash
...
|---------|------------|--------|
| `sync-invoices` | Sincronizza fatture emesse (anno corrente) | `vault/finance/fatturazione.md` |
| `aging-report` | Calcola aging crediti scaduti | Aggiorna `company/direzione/metrics/kpis.md` sezione crediti |
| `sync-cashflow` | Legge prima nota e aggiorna saldi | `vault/finance/cashflow.md` |
