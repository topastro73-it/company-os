...
## Contesto multi-utente (personalizza per il tuo prodotto)

> ⚠️ Adatta i livelli utente al tuo prodotto. Esempio con 3 livelli — se il prodotto è multi-ruolo, i test devono coprirli tutti:

```
Ruolo A — Amministratore/Partner   → gestione, dashboard, configurazione
Ruolo B — Operatore                → demo, onboarding, operatività quotidiana
Ruolo C — Utente finale            → dashboard, report, alert
```
...
**Processo**:
1. Leggi la PRD da `company/prodotto/specs/prd-{slug}.md`
...
## Risorse necessarie
- **Dati di test**: [dataset necessari — record fake, CSV di test, etc.]
- **Account di test**: [ruoli necessari — un account per ogni livello utente]
...
- **Livello utente**: Ruolo A / Ruolo B / Ruolo C
...
- **Request**: token valido ma ruolo con privilegi ridotti (Ruolo C) su endpoint admin
...
**Processo**:
1. Leggi tutte le feature shipped da `company/prodotto/changelog.md`
2. Per ogni feature critica: 2-3 test case core che verificano che funziona ancora
3. Organizza per area: autenticazione, dashboard Ruolo A, modulo operativo (Ruolo B), dashboard utente finale (Ruolo C), API core
4. La regression suite è incrementale — cresce con ogni release

**Output**: `company/prodotto/testing/test-plan-master-regression.md` (file esistente — aggiornamento incrementale, non creare un nuovo `regression-suite.md`)
...
## Autorizzazione
- [ ] Utente finale accede a dati di un altro utente finale → deve fallire
- [ ] Ruolo B (operatore) accede a dashboard admin → deve fallire
...
- [ ] Dati di un tenant/partner non visibili a un altro tenant/partner
...
## Login & Auth
- [ ] Login Ruolo A (admin/partner) funziona
- [ ] Login Ruolo B (operatore) funziona
- [ ] Login Ruolo C (utente finale) funziona
- [ ] Logout funziona
- [ ] Password reset funziona

## Core — Ruolo A (admin/partner)
- [ ] Dashboard admin si carica
- [ ] Lista entità gestite visibile
- [ ] Creazione nuova entità funziona

## Core — Ruolo B (modulo operativo)
- [ ] Flusso operativo principale funziona
- [ ] Output/report generato
- [ ] Export scaricabile

## Core — Ruolo C (utente finale)
- [ ] Dashboard utente finale si carica
...
2. Identifica tutti i dati necessari per eseguire i test:
   ...
   - Dataset (record fake, CSV di test, dati di esempio)
   - Configurazioni (setup tenant/partner, prodotti attivi)
...
```
1. PM scrive PRD             → company/prodotto/specs/prd-{slug}.md
...
### Integrazione ClickUp
I bug trovati durante il testing possono essere creati come task in ClickUp:
- Space/List: vedi `config/integrations.yaml` (ClickUp) → lista Bug
...
- I bug trovati in test vanno in ClickUp con priorità e link al test case
