...
|-------|------|--------------|
| Decisioni pendenti | `company/direzione/decisions/*.md` | Decisioni con `status: open` o `status: pending` |
| Follow-up scaduti | `company/direzione/decisions/*.md`, `company/direzione/reports/follow-ups-*.md` | Checkbox aperti `- [ ]` con owner CEO o senza owner |
| Azioni P0/P1 | `company/direzione/reports/action-plan-*.md` | Azioni con owner CEO o che richiedono approvazione CEO |
| Spec stale | `company/prodotto/specs/INDEX.md` | Spec con status check scaduto (vedi protocollo in CLAUDE.md) |
| Briefing urgenze | `company/direzione/reports/briefing-*.md` (più recente) | Item marcati come urgenti/P0 |
| Roadmap blocchi | `company/prodotto/roadmap.md` | Item bloccati che richiedono decisione CEO |
...
| **Tag** | `from-companyos` + `ceo-action` + tag tipo (es. `decision`, `follow-up`) |

### Step 4 — Genera file di approvazione

Salva in `company/prodotto/clickup-pending/YYYY-MM-DD-sync-ceo-actions.md`:
...
Data: YYYY-MM-DD
Lista target: CEO Personal (<list-id-ceo-personal>)
...
2. Sposta il file in `company/prodotto/clickup-done/`
...
Filtra i task nella lista CEO Personal con tag `from-companyos` e status `complete` / `closed` / `done`:
...
|-----------|-----------|---------------|
| `[DECISION]` | `company/direzione/decisions/*.md` | Aggiorna frontmatter: `status: decided`, aggiungi `decided-date: YYYY-MM-DD` se non presente |
...
| `[REVIEW]` | `company/prodotto/specs/*.md` | Aggiorna frontmatter: `last-status-check: YYYY-MM-DD`, aggiorna `status` se il CEO ha indicato un nuovo status nel commento del task ClickUp |
...
- Aggiorna `company/prodotto/specs/INDEX.md` di conseguenza
...
| 3 | ghi789 — [FOLLOW-UP] Onboarding check | company/direzione/reports/follow-ups-2026-03.md | 2 checkbox marcati come completati |
```
...
3. Aggiorna `company/prodotto/specs/INDEX.md` se spec modificate
...
Quando invocato, il comando controlla anche i task esistenti nella lista CEO Personal con tag `from-companyos`:
