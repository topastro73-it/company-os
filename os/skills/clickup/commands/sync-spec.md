...
Cerca task esistenti con tag `from-companyos` e `spec:{slug}` nella list Epic:
...
```
Epic (in list "Epic" <list-id-epic>): [Titolo Feature]
...
```markdown
## Contesto
[Spiega perché questa feature esiste nel prodotto: quale problema risolve, per quale utente, in quale parte del flywheel si inserisce (ATTRACT/ENGAGE/MANAGE/GROW)]
...
Salva in `company/prodotto/clickup-pending/YYYY-MM-DD-sync-spec-{slug}.md`:
...
|---|------|------|--------|---------|----------|------|
| 1 | Task (Epic) | Epic | — | "Titolo Feature" | high | from-companyos, spec:{slug} |
...
## Tags
Tutti i task avranno tag: `from-companyos`, `spec:{slug}`

## Note
{Eventuali note o assunzioni fatte durante l'estrazione}

## Conferma
Per approvare: `/product clickup approve company/prodotto/clickup-pending/YYYY-MM-DD-sync-spec-{slug}.md`
Per annullare: elimina o ignora questo file.
```

Comunica all'utente: "File di approvazione generato in `company/prodotto/clickup-pending/YYYY-MM-DD-sync-spec-{slug}.md`. Revisa e approva per procedere."
...
1. **Crea task Epic** nella list Epic (`<list-id-epic>`):
   ```
   clickup_create_task(
     name: "Titolo Feature",
...
2. **Crea task Feature** nella list Feature (`<list-id-feature>`):
   ```
   clickup_create_task(
     name: "User Story 1",
...
1. Sposta il file da `clickup-pending/` a `company/prodotto/clickup-done/`
2. Aggiungi al file uno stato finale:
   ...
   Epic task ID: abc123
   ```
3. Aggiorna la spec originale aggiungendo in frontmatter: `clickup-epic: "[{epic_id}](https://app.clickup.com/t/{epic_id})"` — il campo DEVE contenere l'ID e il link diretto all'epic su ClickUp. L'epic si trova nella lista Epics (<list-id-epic>) della board Product Roadmap, ed è collegata ai Feature task tramite dependency di tipo `blocking` (Feature blocks Epic).
4. **Aggiorna la sezione "Implementation Status" della spec** con i task creati:
   ...
   - Se la sezione Implementation Status non esiste nella spec, creala seguendo il template in `os/agents/product/templates/prd.md`
5. **Pubblica/aggiorna la spec su ClickUp Docs**:
   - Ogni spec ha un **Doc dedicato** nel Folder "Product Specs" (ID: `<folder-id-specs>`)
   - Il nome del Doc DEVE essere il titolo della spec (es. `PRD — Assessment Report: Gap Analysis & Remediation Plan`), NON un nome generico
   - Se la spec non ha ancora un Doc (campo `clickup-doc:` vuoto nel frontmatter) → crea con `clickup_create_document` (name = titolo spec, parent = `{"id": "<folder-id-specs>", "type": "5"}`, visibility = `PUBLIC`, create_page = `true`), poi aggiorna la pagina con il contenuto markdown senza frontmatter
