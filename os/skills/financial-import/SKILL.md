...
- **`financial-import`** (questa) = *metodologia* di parsing e analisi di un **export JSON statico**
  (`Company_PRODUCTION.json`) quando l'API non è disponibile: data-dictionary, formule
  MRR/burn/backlog/runway e **regole specifiche dell'azienda** (periodicità compensi, clienti con
  periodicità particolare, grant, finanziamenti bancari). Questa conoscenza di dominio **non** è duplicata in `erp`.

In breve: `erp` porta i numeri (live), `financial-import` sa **interpretarli** (da export). Non si sovrappongono.

Importazione e analisi dei dati finanziari. Supporta due modalità:
1. **ERP API** (preferita) — dati live via `os/skills/erp/SKILL.md` e `scripts/erp_sync.py`
2. **JSON file** (fallback) — export statico `Company_PRODUCTION.json`

Usata da CFO, CEO, Chief of Staff.

## Quando usare questa skill

- **Se l'ERP API è disponibile** (`ERP_API_URL` configurata): usa `python3 scripts/erp_sync.py sync-all` per dati live. Vedi `os/skills/erp/SKILL.md` per dettagli.
...
|---------|------------|--------|
| `import-financials` | Importa JSON finanziario, analizza, aggiorna KPI | Aggiorna `company/direzione/metrics/kpis.md` + report |
...
### Input
- File JSON allegato dall'utente (formato export dell'azienda)
...
#### Passo 3 — Regole specifiche dell'azienda

**CRITICO — Periodicita compensi**:
- **Solo alcune persone** hanno stipendio mensile (identificale nei financials tramite il campo `resource`)
- Gli altri compensi (es. CEO, CTO, collaboratori) sono spesso trimestrali o a progetto
...
**Clienti senza contratto — Revenue non contrattualizzata**:
- Alcuni clienti generano revenue mensile variabile ma potrebbero non avere un `contract` nel sistema
- Cercala nei `financials[]` filtrando per il `resource` del cliente
...
**Scelta operativa** (vedi `decisions/`): la pipeline commerciale si legge da **HubSpot CRM** (via MCP tool `search_crm_objects`), NON dal JSON. Il JSON ha dati pipeline stale e incompleti.
...
**Stage mapping HubSpot** (gli ID reali degli stage sono in `config/integrations.yaml`; qui il mapping logico):
