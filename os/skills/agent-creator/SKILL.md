...
- `os/agents/AGENTS.md` — per capire gli agenti esistenti ed evitare duplicati o conflitti di ruolo
- `os/skills/SKILLS.md` — per conoscere le skill disponibili da assegnare al nuovo agente
...
→ Claude mappa su file esistenti in `company/`, `vault/`, `zones/_root/context/`. Se un dato non esiste ancora, lo segnala come "da creare".
...
→ Claude propone una selezione dalle skill in `os/skills/SKILLS.md` pertinenti al ruolo. L'utente approva o aggiusta.
...
1. **`os/agents/{slug}/AGENT.md`** — usando il template sotto
2. **`os/agents/{slug}/COMMANDS.md`** — indice comandi con slug, descrizione, path output
3. **`os/agents/{slug}/commands/`** — una sottocartella vuota (i file dei singoli comandi vengono aggiunti alla prima esecuzione di ciascun comando)
4. **Aggiorna `os/agents/AGENTS.md`** — aggiunge la riga nella tabella agenti
...
1. `zones/_root/context/COMPANY.md` — Value proposition e contesto aziendale
...
- {`os/skills/{slug}/SKILL.md` — descrizione uso}
