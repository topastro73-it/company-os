...
| `stripe-invoices [anno]` | Lista fatture filtrate per anno con totali | Report in `vault/finance/reports/` |
| `stripe-mrr` | Calcola MRR da subscription attive | Aggiorna `company/direzione/metrics/kpis.md` sezione MRR |
| `stripe-customers` | Anagrafica clienti Stripe con subscription status | Cross-ref con `20-Clienti/` |
...
- **Fallback**: se il connector MCP non è disponibile, i dati più recenti sono in `vault/finance/` (ultimo sync). Segnalare al CEO che Stripe MCP è offline.
- **Cross-reference**: i clienti Stripe (cus_*) vanno mappati con le schede partner in `20-Clienti/` e con i contatti HubSpot CRM.
